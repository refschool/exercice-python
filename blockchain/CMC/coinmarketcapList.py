...
for crypto in list(latest):
    ...
    liste = [crypto['name'] + ' - ' + crypto['symbol'], float(quote['price']),float(quote['percent_change_1h']),float(quote['percent_change_24h']),float(quote['percent_change_7d'])]
    arr.append(liste)
...
""" 1 sort by hourly, 2 sort by daily 24h """
sortedlist = sorted(arr,key=lambda x:x[H1])
sortedlist = sortedlist[::-1]
""" enregistrement du fichier brut """
date = datetime.datetime.now()
date = date.strftime("%Y%m%d-%H%M")
filename = 'dump/cmc-' + date

import pickle
with open(filename,'wb') as fichier:
    pickle.dump(sortedlist,fichier)

print(tabulate(sortedlist,headers=['Crypto','Price','Variation H1','Variation H24','Variation 7d']))